...
//shows 9 people on the page and puts everything together
function runPage(page) {
   ...
   //this is to set the display checks if its page 5 and subtract 3 from lastIndex to fix error 
   if (button.tagName === 'BUTTON' && parseInt(button.textContent) === 5) {
      for (let i = 0; i < data.length; i++) {
         LIfullStudentlist[i].style.display = 'none';
      }
      for (i = firstIndex; i <= lastIndex - 3; i++) {
         LIfullStudentlist[i].style.display = '';
      }
   } else {
      for (let i = 0; i < data.length; i++) {
...
//adds button click event to run page function 
